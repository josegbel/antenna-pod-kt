...
| **d** | `error.cause!!.message` (Gap 16) — reachable wrong-password crash | **PRESERVED here, pinned executably for the first time, fixed in Milestone 18** | See **D11**. |
...
- [ ] **AC14** — `grep -F -c 'error.cause!!.message' GpodderAuthenticationFragment.kt` → **1**, `grep -F -c 'error.message' …` → **1**, `grep -F -c 'error.cause?.message' …` → **0**. Milestone 15's two-handler asymmetry survives the conversion intact and is now backed by two executable tests rather than a grep alone.
...
- **Fixing Gap 16** (D11, OQ1). The wrong-password NPE ships preserved and newly pinned. The one-line fix and its test are written into Milestone 18's brief.
...
| | Question | Status |
|---|---|---|
| **OQ1** | Gap 16 ships as a preserved, pinned crash | **Open — for José. Non-blocking; a decision either way changes one line and one test.** |
| **OQ2** | `lifecycle-runtime-compose` version ref is a misnomer | Open. Own micro-task. Does not block. |
| **OQ3** | Upstreaming intent (standing, carried from M7/M9/M11–M16) | Open, standing. Does not block. |
...
| d | `error.cause!!.message` (Gap 16) — reachable wrong-password crash | **PRESERVED here, pinned executably, fixed in Milestone 18** | The statement is verbatim in the converted `catch`. `testWrongPasswordErrorPathThrowsFromNullCause` green at Steps 3/7/8. **Exception-type difference stated explicitly:** on RxJava the throw from the onError consumer reaches the handler wrapped in a `CompositeException`; the coroutine rewrite propagates a bare `NullPointerException` through `lifecycleScope.launch` to the thread's uncaught handler. The *crash* is preserved; the wrapper type is not. `grep -F -c 'error.cause!!.message'` → **1**, `'error.message'` → **1**, `'error.cause?.message'` → **0** (AC14). |
...
| File | Step 1 | Step 9 |
|---|---|---|
| `SynchronizationPreferencesFragment.kt` | 30 | **22** |
| `GpodderAuthenticationFragment.kt` | 12 | **12** |
| `NextcloudAuthenticationFragment.kt` | 7 | **7** |
| `AuthenticationDialog.kt` | 0 | **0** |
| **slice total** | **49** | **41** |
...
| AC13 | PASS (local) | 22 / 12 / 7 / 0 = 41; `supportActionBar` grep → 1. |
| AC14 | PASS (local) | `error.cause!!.message` → 1, `error.message` → 1, `error.cause?.message` → 0. |
