...
- **AC13 — CANNOT BE SATISFIED IN ITS LITERAL FORM, for a reason unrelated to this implementation. Documented rather than claimed or silently dropped.**
  ...
  **RESOLVED 2026-08-20 — the pre-existing static-analysis debt named above was cleared by four sibling follow-up tasks** (`antennapod-fix-spotbugs-static-analysis-debt` PR #23, `antennapod-fix-net-download-service-spotbugs-debt` PR #25, `antennapod-fix-app-wearos-compile-error` PR #26, plus two more discovered afterward: `antennapod-fix-httpdownloadertest-null-bundle-argument` PR #27 and `antennapod-fix-feeditemlistfragment-null-feed-crash` PR #29 with its own follow-up `antennapod-fix-feeditemlistfragmenttest-flaky-characterization-tests` PR #30 — all merged into `develop`). This branch was merged with `origin/develop` (merge commit `412068b6d`, no conflicts, no rebase/force-push needed) and pushed, triggering CI run `32348999145`.

  **`static-analysis` passed for the first time ever on this PR.** `unit-test` and `emulator-test` both ran for real (not skipped) and passed on all variants (`FreeRelease`, `PlayDebug`, `PlayRelease`) and all three `emulator-test` API legs (23/30/36). `SyncSettingsScreenshotCaptureTest`'s all three test methods **PASSED** on `ubuntu-latest` (job `96364870100`, `Unit Test: PlayDebug`):
  - `testCapturedBitmapIsNotBlankAndHasExpectedDimensions` — **PASSED.** The test's dimension checks are `assertEquals(822, bitmap.width)` / `assertEquals(1782, bitmap.height)` (exact-match, not range) — a PASS here is a deterministic confirmation the observed dimensions on Linux CI are exactly **822×1782**, matching every local macOS probe. This is the specific cross-machine-determinism gap Research's Characterization gap 3 flagged as unverified — now verified.
  - The same test's distinct-colour-count check is `assertTrue(distinctColors.size >= 200)` — a threshold, not an equality, and the test has no `println`/log statement for the actual count. **PASSED confirms the observed count is ≥ 200, but the exact observed number was never captured anywhere retrievable** (no artifact was uploaded for this run — only the APK; the PNG only exists ephemerally on the runner's `build/reports/screenshots/` under `:ui:preferences`). AC13's text asks for "the distinct-colour count... the runner observed" — that literal number cannot be pasted here because the test as written doesn't produce it, not because it wasn't looked for. Flagging honestly rather than inventing a figure or overstating an inequality check as an equality one.
  - `testFirstPreferenceRowIsNotClippedByActionBar` — **PASSED** (the real `ToolbarActivity` theme/content-id fix from Plan rev. 1/Red-team Loop 1 holds on Linux CI too).
  - `testWritesPngUnderModuleBuildDirectory` — **PASSED**, log line: `Screenshot written to: /home/runner/work/antenna-pod-kt/antenna-pod-kt/ui/preferences/build/reports/screenshots/sync_settings_before_milestone_15b.png`.

  **AC13's substance — do the screenshot test's environment-sensitive assertions pass on Linux CI — is now satisfied with real evidence for two of its two checks' pass/fail outcomes, and for the dimension check's exact value.** The one gap is literal-textual, not substantive: the plan text anticipated a printed colour-count number that the shipped test never emits. Whether that gap needs a follow-up (a one-line `println(distinctColors.size)` added and CI re-run) or whether "the ≥200 threshold held on real Linux CI" is sufficient to close AC13 as originally intended is a José call — no code change was made to the test in resolving this, per this task's own scope discipline.
