...
### 11. `SynchronizationQueueStorage.removeLegacyConflictingFeedEntries`'s `List.toString()`-into-JSON defect (pinned, not fixed, in Milestone 12) — needs a data migration
**Raised:** 2026-08-02, Milestone 12 (`:net:sync:service`) research/plan (D9, OQ3).
**Status:** Deferred — proposed as its own future task, not part of this milestone.

`SynchronizationQueueStorage.kt` (`removeLegacyConflictingFeedEntries`) writes `Collection.toString()` — e.g. `"[https://a.example/feed.xml, https://b.example/feed.xml]"`, unquoted and comma-space separated — into the `sync_added`/`sync_removed` SharedPreferences keys, which every other read path parses with `org.json.JSONArray`. Real feed URLs contain `://`, which `JSONTokener` cannot parse leniently (unlike a toy `"a"`/`"b"` fixture), so the parse throws `JSONException`, which is swallowed via `printStackTrace()`, silently emptying both queues. Unlike Milestone 11's `EpisodeAction.equals` defect, this one is **live**: it is reached from `SyncService.syncSubscriptions`'s `catch (SyncServiceException)` block whenever a subscription upload fails server-side, which is not a rare or contrived path. It is pinned exactly as-is by three tests in `SynchronizationQueueStorageTest.java` (`removeLegacyConflictingFeedEntriesWritesUnquotedListToStringIntoBothKeys`, `…LeavesBothQueuesUnreadableForRealFeedUrls`, `…RoundTripsWhenValuesAreJsonTokenerSafe`), the last of which documents *why* the fixture must use real URLs — Android's JSON tokener is lenient enough that a naive test fixture would round-trip successfully and pin nothing.

Fixing the write is a one-line change (write valid JSON, not `toString()`); fixing the **installed base** is not, because devices already carrying a corrupt value will keep failing to parse it until something overwrites the key with valid JSON — a data migration, not just a bugfix.

**Recommendation when picked up:** a small write-side fix plus a startup-time (or first-read) migration that detects and repairs an unparseable value in either key. Whether it ships as its own standalone bugfix PR (the natural pairing if deferred initiative #2's "upstream vs. fork" question ever resolves toward upstream) or stays a recorded-but-unfixed defect in the fork is a José call per root `CLAUDE.md`'s commercial-implications rule, not an agent call — see `tasks/antennapod-net-sync-service-kotlin.md`'s OQ1/OQ3.

### 12. `androidx.work:work-testing` is not in the version catalog — `SyncService.doWork()` converted without behavioural coverage
**Raised:** 2026-08-02, Milestone 12 (`:net:sync:service`) research/plan (D12, OQ2).
**Status:** Deferred — a repo-wide tooling decision, not this milestone's to make.

`SyncService`'s 371 LOC (46% of the module) — `doWork()`'s five-outcome control-flow matrix, `syncSubscriptions`, `syncEpisodeActions`, `processEpisodeActions`, the notification paths, `getActiveSyncProvider`, and `SynchronizationQueueImpl`'s five WorkManager-touching methods — converted under inspection-level proof (an exact `!!` inventory, a line-referenced transcription contract, two `javap -c` bytecode checks, and a reflection-based WorkManager contract test) rather than behavioral test coverage, because `androidx.work:work-testing` is declared in no `gradle/libs.versions.toml` anywhere in the repo and adding it would be a repo-wide dependency change, out of File Scope for a single-module milestone. Worth noting honestly: even with `work-testing`, `TestListenableWorkerBuilder` would reach only 2 of the control-flow matrix's 5 outcomes here, because `getActiveSyncProvider()` constructs its network backends (`GpodnetService`/`NextcloudSyncService`) concretely with no injection point and the module declares no mocking framework — everything past `activeSyncProvider.login()` stays out of reach without a design change (extracting an injectable `ISyncService` factory), which is its own, separate initiative.

**Recommendation when picked up:** adding `work-testing` to the version catalog would also unlock behavioral testing for `:net:download:service`'s `EpisodeDownloadWorker`/`FeedUpdateWorker` and `:storage:database-maintenance-service`'s periodic work — a better argument for taking it repo-wide than this module alone makes. Scope it as its own small cross-cutting task (add the dependency, verify it against one `Worker` first) before assuming any specific module will consume it.

### 13. Dead dependencies in `:net:sync:service` and its dead consumer in `:ui:preferences`
**Raised:** 2026-08-02, Milestone 12 (`:net:sync:service`) research/plan (D15, OQ4).
**Status:** Deferred — logged rather than taken, per the plan's own decisions; same shape as deferred initiative #10.

Four independent, low-stakes items bundled here:
- `net/sync/service/build.gradle` declares `libs.okhttp`, `libs.rxandroid`, and `libs.guava` with zero references anywhere in `src/` (verified by grep for `okhttp3`, `io.reactivex.rxjava3.android`/`AndroidSchedulers`, and `com.google.common`). `libs.rxjava` itself is genuinely used (by `LockingAsyncExecutor`) and is not part of this item.
