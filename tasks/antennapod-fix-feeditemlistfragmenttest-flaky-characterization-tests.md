...
### Verification performed (independent, not taken on the developer's report)

- **Layout XML claims, read directly rather than trusted.** `app/src/main/res/layout/feed_item_list_fragment.xml`: confirmed `<ProgressBar android:id="@+id/progressBar">` (`:65-71`) is a direct child of the root `<androidx.coordinatorlayout.widget.CoordinatorLayout android:id="@+id/coordinatorLayout">` (`:2-87`), a sibling of `appBar`, `swipeRefresh`, `more_content`, and `floatingSelectMenu`. `grep -rl 'id="@+id/appBar"'` → exactly 2 layouts (`feed_item_list_fragment.xml`, `feedinfo.xml` — a different fragment). `grep -rl 'id="@+id/coordinatorLayout"'` → exactly 1 layout. `grep -rl 'id="@+id/progressBar"'` → exactly 14 layouts, matching D6's count. Read `feeditemlist_header.xml` in full: exactly one `txtvTitle` (`:136`), no `<include>` inside it, confirming the `appBar` → `header` → `txtvTitle` chain is unique. Read `nav_list.xml` in full: its `ProgressBar` (`:76-81`) is a direct child of the root `RelativeLayout android:id="@+id/nav_layout"`, not `coordinatorLayout` — `withParent(withId(R.id.coordinatorLayout))` correctly excludes it. Read `feeditemlist_item.xml:180-210`: its `LinearProgressIndicator android:id="@+id/progressBar"` is nested several `LinearLayout`s deep inside an episode row, nowhere near a `CoordinatorLayout` — also correctly excluded. All D2/D3/D6 matcher-uniqueness claims hold under direct inspection, not just the plan's/red-team's/developer's assertion of them.
- **File Scope.** `git diff a6275e8f0..e8069a441 --stat -- . ':!tasks' ':!features'` → exactly 2 files: `FeedItemlistFragmentTest.java` (+8/-2 across both commits) and `FeedItemlistFragment.java` (+2/-0). `git status --short` on the branch is clean. No file outside the Plan's File Scope was touched by the code commits (the docs commit `e8069a441` touches only this task's own `tasks/`/`features/` files, confirmed via `git show e8069a441 --stat`).
- **Production diff correctness.** Read `FeedItemlistFragment.java:652-701` (current state) directly. The two added `viewBinding.progressBar.setVisibility(View.GONE);` lines sit in the `error` consumer and the `onComplete` handler only, each immediately after `refreshHeaderView();` and before `adapter.setDummyViews(0);` — byte-identical positioning to the success handler's pre-existing equivalent line. `git show ecd85e9ca` confirms `2 insertions(+), 0 deletions(-)` for this file, no other line touched. `git diff --stat` matches the Implementation Notes' own reported byte counts exactly.
- **No interaction with the parent task's `localFeed`/`MaybeCallbackObserver` concurrency finding.** Read the full `loadItems()` body. That finding is scoped to the success consumer's `Feed localFeed = feed;` capture and the try/catch escalation behavior of `MaybeCallbackObserver.onSuccess`. The two new lines are in `error`/`onComplete`, never touch `localFeed`, and the success handler's block (`:668-684`) is byte-for-byte unmodified — confirmed by reading it end to end, not just diffing.
- **Test assertions are real, not just invocations.** Read the full current `FeedItemlistFragmentTest.java`. Every new/changed line ends in `.check(matches(...))` against a concrete expected value: `withText("")`, `not(isDisplayed())`, `withText(feed.getTitle())` + `isDisplayed()`, `hasChildCount(0)`/`hasChildCountAtLeast(1)`. None of the new assertions merely invoke a matcher without asserting on it. The `@Rule`, `testExistingFeedLoadsItems`'s setup/data, the `Thread.sleep(1000)` at `:56` (renumbered from `:53`), and both private `hasChildCount*` helpers are confirmed untouched, exactly as File Scope required.
- **Compile and static checks, re-run rather than trusted.** `./gradlew :app:compilePlayDebugAndroidTestSources :app:compileFreeDebugAndroidTestSources` → BUILD SUCCESSFUL, both flavors. `./gradlew checkstyle lint` → BUILD SUCCESSFUL, no findings attributable to either changed file. Both independently reproduce the developer's reported results.
- **AGENTS.md compliance.** No comments added to either file (confirmed by reading both diffs in full). Diff is minimal — exactly the statements the Plan specified, no reordering, no renaming, no unrelated cleanup.
- **Runtime verification — partial, with an honestly-disclosed gap.** I attempted to independently boot the `Pixel_9a` AVD in this review environment to re-run `FeedItemlistFragmentTest` myself, going beyond static verification. The emulator failed to launch here: `Could not launch '.../qemu/darwin-x86_64/qemu-system-aarch64': No such file or directory` — this review sandbox's Android SDK emulator package is incomplete, an environment limitation on my end, not a defect signal about the diff. I could not independently reproduce the developer's reported real-device API 36 pass/fail. This does not block APPROVE: (a) the developer's report is internally consistent and detailed (2/2 before Step 2, 2/2 after, matching the AVD `Pixel_9a` that does exist on this machine per `emulator -list-avds`); (b) the actual authoritative runtime gate is Step 3 — three full CI `emulator-test` runs across all three API levels — which is correctly still unperformed and correctly not being claimed as done anywhere in the Implementation Notes or Acceptance Criteria checkboxes; (c) every claim that *is* independently verifiable without a device (layout containment, diff shape, compile, lint) checks out. Filed as MINOR below for transparency, not as a defect.

### Findings

- **Severity:** MINOR
  **Class:** Tests
  **File:line:** N/A (review-environment infrastructure, not a code location)
  **Finding:** This review could not independently boot an emulator to re-execute `FeedItemlistFragmentTest` at runtime (missing `qemu-system-aarch64` in this sandbox's emulator package), so the developer's reported real-device API 36 before/after results are corroborated circumstantially (matching AVD present, internally consistent report, compile+lint independently reproduced) rather than re-executed by the reviewer.
  **Suggested fix:** None required of the developer — this is a reviewer-environment gap. Step 3's mandatory 3×-full-matrix CI run remains the authoritative verification and is correctly still pending; ensure it runs before any merge recommendation, as the Plan already requires.

### Acceptance Criteria walk-through (this loop's scope)

Criteria verifiable without CI access — all satisfied:
- [x] Diff to `FeedItemlistFragment.java` is exactly 2 added lines, both the exact statement specified, no other line touched — confirmed via `git show` and direct read.
- [x] `error` consumer's added line positioned identically to `onComplete`'s and to the success path's existing line — confirmed by direct read.
- [x] No public API break — `loadItems()` remains `private`, both handlers remain lambdas, no signature/field/visibility change — confirmed, no declaration-line changes in the diff.
- [x] `./gradlew checkstyle lint` passes — reproduced independently.
- [x] Diff touches exactly the two File-Scope files, nothing else — confirmed via `git diff --stat` excluding docs.
- [x] No comments added to either file — confirmed.

Criteria correctly deferred to Step 3 (not this review's job to tick, and Implementation Notes do not claim them done):
- [ ] Both tests pass on API 23/30/36, first attempt-leg, no `AmbiguousViewMatcherException`, no 45s timeout, progressBar assertion green on all three levels.
- [ ] Full `emulator-test` suite green on all three legs.
- [ ] Step 3's three CI runs completed and recorded; merge not recommended until then.

### Summary

The implementation matches the Plan's Steps 1 and 2 exactly — verified against the actual layout XML (not the report of it), the actual production code (not the report of it), and the actual test file (not the report of it). Both matcher fixes and the `progressBar` anchor are structurally unique by direct inspection, not merely plausible. The two-line production fix is minimal, correctly positioned, and demonstrably does not reopen or touch the parent task's already-twice-red-teamed `localFeed`/success-handler concurrency region. New test assertions genuinely assert on concrete view state rather than merely invoking matchers. File Scope, AGENTS.md's no-comments/minimal-diff rules, and every diff-shape Acceptance Criterion checkable without CI access are satisfied. The mandatory Step 3 CI-verification gate is correctly still outstanding and correctly not represented as complete anywhere in the Implementation Notes. Proceed to `legacy-android-red-team` (implementation-stage review).
...
### Verification performed (independent, not taken on the prior stages' reports)

Re-derived every claim from the actual commits (`734c0971a`, `ecd85e9ca`) and the current file contents, not from the Plan's or code review's summaries of them.

- **`git show 734c0971a` and `git show ecd85e9ca`** read in full. Diff shape matches the Plan byte-for-byte: commit 1 is `+3/-2` in the test file only (both matcher lines plus the one added import), commit 2 is `+3/-0` in the test file (new progressBar assertion plus two imports) and `+2/-0` in `FeedItemlistFragment.java` (the two `View.GONE` lines, positioned identically after `refreshHeaderView();` and before `adapter.setDummyViews(0);` in both the `error` and `onComplete` handlers). No other file touched by either commit.

- **Question 1 — progressBar matcher robustness.** Read `app/src/main/res/layout/feed_item_list_fragment.xml` directly: `<ProgressBar android:id="@+id/progressBar">` (`:65-71`) is a static, single direct child of the root `<CoordinatorLayout android:id="@+id/coordinatorLayout">` (`:2-87`), sibling to `appBar`, `swipeRefresh`, `more_content`, `floatingSelectMenu`. Grepped `id="@+id/coordinatorLayout"` → exactly 1 layout repo-wide; `id="@+id/progressBar"` → exactly 14 layouts. Read the two collision layouts directly: `nav_list.xml:76-81`'s `ProgressBar`'s immediate parent is `RelativeLayout android:id="@+id/nav_layout"`; `feeditemlist_item.xml:198-205`'s `LinearProgressIndicator android:id="@+id/progressBar"` sits several `LinearLayout`s deep inside an episode row — neither is a `withParent(withId(R.id.coordinatorLayout))` match. Went further than the prior two reviews on one point: grepped `FeedItemlistFragment.java` for `coordinatorLayout`/`Snackbar` usage — zero hits, so nothing in the fragment's own code dynamically attaches a view to `coordinatorLayout` at runtime that could introduce a second `progressBar`-id sibling (e.g. a Snackbar anchored there). Confirmed no flavor-specific override of `feed_item_list_fragment.xml` exists (`app/src/play/res` has no matching file; only `app/src/main/res` defines it) and no `layout-v*`/`layout-sw*` qualifier touches this file. The view tree here is static XML with no runtime restructuring in scope. The matcher anchors correctly and I found no state under which it could break.

- **Question 2 — interaction with the parent task's `localFeed`/`MaybeCallbackObserver` concurrency finding.** This is where I pushed past what the code review and plan-stage red-team verified. Traced the `viewBinding` field's full lifecycle rather than trusting the "different consumer, no interaction" framing at face value: `viewBinding` is assigned exactly once, in `onCreateView` (`:133`), and — confirmed by grepping the whole file for `viewBinding = ` — is **never reassigned or nulled anywhere**, including `onDestroyView` (`:256-264`, which only disposes `disposable`, unregisters EventBus, and calls `adapter.endSelectMode()`). This matters because `refreshHeaderView()` (`:496-500`) carries a defensive `if (viewBinding == null || feed == null) return;` guard that, given the above, is dead code in the current lifecycle implementation — it can never observe `viewBinding == null`. I chased this because it raised a real question: if that guard is dead, could the two newly-added `viewBinding.progressBar.setVisibility(View.GONE);` lines — which have no such guard of their own — NPE on a torn-down view? Traced all three `loadItems()` call sites (`:197` in `onCreateView`, after `viewBinding` is assigned; `:397` in an `onEvent(FeedEvent)` EventBus handler; `:472` in `updateUi()`) — all three only fire while the fragment's view is alive, so `viewBinding` is always a real (if possibly stale/detached) binding, never `null`, by the time any of the three `loadItems()` terminal handlers execute. Also confirmed `loadItems()` disposes any prior `disposable` before creating a new one (`:653-655`), so overlapping subscriptions cannot exist, and RxJava3's `MaybeCallbackObserver` wraps `onSuccess`/`onError`/`onComplete` identically (each try/catches its consumer and escalates to `RxJavaPlugins.onError` on throw) — so there is nothing structurally different about the `error`/`onComplete` paths that would make them more crash-prone than the already-shipped `:678` success-path line this diff mirrors. The two new lines are idempotent, read no shared mutable field (they don't touch `feed`, which is what the parent finding's `localFeed` capture is about), and cannot race with the success handler's own `:678` call since exactly one terminal event fires per subscription. **Conclusion: no interaction, and no new hazard** — but for a reason one level more specific than "different consumer": `viewBinding` genuinely cannot be null at any call site in this codebase today, so the theoretical NPE-on-torn-down-view scenario I went looking for doesn't materialize. Filed as MINOR below since the dead guard at `:496` is a pre-existing, out-of-File-Scope oddity worth a future look, not a defect in this diff.

- **Question 3 — matchers passing "for the wrong reason."** Independently re-derived the uniqueness proof rather than re-reading the prior reviews' conclusions: `appBar` is declared in exactly 2 layouts repo-wide (`feed_item_list_fragment.xml:11`, `feedinfo.xml:11` — a different, never-concurrent fragment). Within `feed_item_list_fragment.xml`, `appBar`'s content chain is `imgvBackground` (ImageView, no `txtvTitle`) → `header` include → `feeditemlist_header.xml` (confirmed exactly one `txtvTitle`, at `:136`, via direct grep of the file, no nested `<include>`) → `toolbar` (`MaterialToolbar`, no `txtvTitle` id). Confirmed no flavor-specific override of either `feed_item_list_fragment.xml` or `feeditemlist_header.xml` exists in `app/src/play/res`. So the `:59` matcher is not "happening to pass" — it is structurally forced to match exactly the intended header title view, on all three API levels, since the containment relationship is static XML with no qualifier variance. Same conclusion for the `:60` progressBar anchor (Question 1). No "passing for the wrong reason" risk found for either new/changed assertion.

- **Question 4 — Step 1 / Step 2 interaction.** The `:59` title-under-`appBar` check and the new `:60` progressBar-under-`coordinatorLayout` check target disjoint views with disjoint matcher trees and no shared state; they are independent, sequential Espresso assertions, and their relative order has no bearing on correctness (swapping them would behave identically). Confirmed `734c0971a`'s matcher fix is unconditional — no API-level branching, no flag — so it is fully in effect before `ecd85e9ca` lands, meaning Step 2 cannot expose an unfixed Step 1 defect on any code path. No ordering or independence risk.

### Checklist categories considered

1. **Characterization tests prove equivalence, not existence** — Step 1's matchers pin real, structurally-unique behavior (re-verified independently, not on faith); Step 2's new assertion is a genuine regression-test proof that the `onComplete` handler's `View.GONE` call takes effect (developer's real-device run + code review's independent compile verification; still gated on Step 3 for the `error` path and for API 23/30 coverage, correctly not claimed done).
2. **Silent behavior changes from mechanical translation** — not applicable, no J2K conversion, Java-only diff.
3. **Public API breakage** — dismissed: `loadItems()` remains `private`, no signature/field/visibility change in either file; confirmed by reading both diffs in full, not just the Acceptance Criteria checkbox.
4. **Coverage gaps left unaddressed** — the `error` branch's progressBar fix remains unasserted, honestly disclosed as Out of Scope in the Plan; not a silent gap.
5. **`concurrency` track** — not requested; the deep dive above is the substantive check for this task, and it clears.
6-8. **`compose`/`navigation`/`di`/`gradle-kts`** — not applicable; confirmed the diff touches only a Java View/XML fragment and a Java Espresso test.
9. **Milestone/scope creep** — dismissed. `git show --stat` on both commits confirms exactly the two File-Scope files, exact line counts the Plan specified.

### Concerns

- **Severity:** MINOR
  **Class:** Coverage gaps left unaddressed (pre-existing, out of File Scope)
  **Concern:** `refreshHeaderView()`'s `if (viewBinding == null || feed == null)` guard at `FeedItemlistFragment.java:496` is dead code under the current lifecycle implementation — `viewBinding` is assigned once in `onCreateView` and never nulled anywhere in the class (including `onDestroyView`), so the `viewBinding == null` branch can never be taken. This is not introduced by this diff and not in File Scope to fix here, but it's worth flagging because it's exactly the kind of defensive-looking check that could mislead a future reader (or a future J2K/`kotlin`-track conversion) into believing `viewBinding` can legitimately be null at that point, when in this codebase today it cannot.
  **Evidence:** `FeedItemlistFragment.java:256-264` (`onDestroyView` — no `viewBinding = null`), `:496-500` (the guard), confirmed via `grep -n "viewBinding = " FeedItemlistFragment.java` returning only the `onCreateView` assignment at `:133`.
  **Suggested mitigation:** No action required for this task. Worth a note for whoever next touches `FeedItemlistFragment.java` (e.g. a future `kotlin`-track conversion of this fragment) — either the guard is genuinely dead and could be simplified, or there's a lifecycle scenario (process-death restore, retained-instance edge case) that should null `viewBinding` in `onDestroyView` and currently doesn't. Not this task's problem to resolve.
